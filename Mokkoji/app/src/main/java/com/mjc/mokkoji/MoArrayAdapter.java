...
import com.mjc.mokkoji.MainActivity.ImageItemClickListener;

import java.util.List;

public class MoArrayAdapter extends ArrayAdapter<MoItem> {
    ...
    public MoArrayAdapter(Context a_context, List<MoItem> a_itemList) {
    ...
    public View getView(int a_position, View a_convertView, ViewGroup a_parent) {
        MoItemViewHolder viewHolder;
        if (a_convertView == null) {
            a_convertView = LayoutInflater.from(mContext).inflate(LAYOUT_RESOURCE_ID, a_parent, false);

            viewHolder = new MoItemViewHolder(a_convertView);
            a_convertView.setTag(viewHolder);
        } else {
            viewHolder = (MoItemViewHolder) a_convertView.getTag();
        }

        final MoItem MoItem = mItemList.get(a_position);

        // Icon 설정
        viewHolder.ivIcon.setImageResource(MoItem.getImageResId());
        viewHolder.ivIcon.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View a_view) {
                if (mImageItemClickListener != null) {
                    mImageItemClickListener.onImageItemClick(MoItem.getImageResId());
                }
            }
        });

        // Country 설정
        viewHolder.tvItem.setText(MoItem.getMoItem());
